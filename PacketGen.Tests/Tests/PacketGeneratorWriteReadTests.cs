...
internal class PacketGeneratorWriteReadTests
{
    [Test]
    public void WriteRead_PrimitivesAndArrays_RoundTrip()
    {
        string testCode = BuildPrimitivesAndArraysSource();

        GeneratedAssemblyHarness harness = GeneratedAssemblyHarness.Build<PacketGenerator>(testCode, "CPacketPrimitivesArrays.g.cs");
        Type packetType = harness.GetTypeOrFail("TestPackets.CPacketPrimitivesArrays");

        object packet = PacketReflectionHelper.CreatePacketInstance(packetType);
        foreach (PropertyCase testCase in BuildPrimitiveArrayCases())
        {
            PacketReflectionHelper.SetProperty(packet, testCase.Name, testCase.Value);
        }

        object writer = harness.CreateWriter();
        Type readerType = harness.GetTypeOrFail("PacketReader");
        ...
        foreach (PropertyCase testCase in BuildPrimitiveArrayCases())
        {
            object? actual = PacketReflectionHelper.GetProperty(roundTripPacket, testCase.Name);
            DeepAssert.AreEqual(testCase.Value, actual, testCase.Name);
        }
    }

    [Test]
    public void WriteRead_GenericsAndNested_RoundTrip()
    {
        string testCode = BuildGenericsSource();

        GeneratedAssemblyHarness harness = GeneratedAssemblyHarness.Build<PacketGenerator>(testCode, "CPacketGenerics.g.cs");
        Type packetType = harness.GetTypeOrFail("TestPackets.CPacketGenerics");

        object packet = PacketReflectionHelper.CreatePacketInstance(packetType);
        foreach (PropertyCase testCase in BuildGenericCases())
        {
            PacketReflectionHelper.SetProperty(packet, testCase.Name, testCase.Value);
        }
